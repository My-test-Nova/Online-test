...
class OnlinePauseSubState extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		if (!OnlinePlayState.instance.startingSong)
		...
		if (!OnlinePlayState.chartingMode)
		...
		/*var textString:String = Date.now().toString() + '\n' +
			'Song: ' + OnlinePlayState.SONG.song + ' - ' + Difficulty.getString().toUpperCase() + '\n' +
			'Blueballed' + OnlinePlayState.deathCounter + '\n' + 
			(OnlinePlayState.instance.practiceMode ? 'Practice: ON\n' : '') +
			(OnlinePlayState.instance.cpuControlled ? 'Botplay: ON\n' : '') +
			(OnlinePlayState.chartingMode ? 'Cheating: ON'); */
		...
		songText = new FlxText(0, 15, 0, OnlinePlayState.SONG.song + ' - ' + Difficulty.getString().toUpperCase(), 32);
		songText.setFormat(font, 25);
		songText.antialiasing = ClientPrefs.data.antialiasing;
		songText.updateHitbox();
		add(songText);

		ballText = new FlxText(0, 15, 0, 'Blueballed: ' + OnlinePlayState.deathCounter, 32);
		ballText.setFormat(font, 25);
		ballText.updateHitbox();
		ballText.antialiasing = ClientPrefs.data.antialiasing;
		add(ballText);

		practiceText = new FlxText(0, 15, 0, 'Practice Mode: ' + (OnlinePlayState.instance.practiceMode ? 'ON' : 'OFF'), 32);
		practiceText.setFormat(font, 25);
		practiceText.updateHitbox();
		practiceText.antialiasing = ClientPrefs.data.antialiasing;
		add(practiceText);

		botText = new FlxText(0, 15, 0, 'Botplay: ' + (OnlinePlayState.instance.cpuControlled ? 'ON' : 'OFF'), 32);
		botText.setFormat(font, 25);
		botText.updateHitbox();
		botText.antialiasing = ClientPrefs.data.antialiasing;
		add(botText);

		cheatingText = new FlxText(0, 15, 0, 'Cheating: ' + (OnlinePlayState.chartingMode ? 'ON' : 'OFF'), 32);
	...
	override function update(elapsed:Float)
	{
		...
		songText.text = OnlinePlayState.SONG.song + ' - ' + Difficulty.getString().toUpperCase();

		ballText.text = 'Blueballed: ' + OnlinePlayState.deathCounter;

		practiceText.text = 'Practice Mode: ' + (OnlinePlayState.instance.practiceMode ? 'ON' : 'OFF');

		botText.text = 'Botplay: ' + (OnlinePlayState.instance.cpuControlled ? 'ON' : 'OFF');

		cheatingText.text = 'Chart editor debug: ' + (OnlinePlayState.chartingMode ? 'ON' : 'OFF');
	...
	function changeOptions(num:Int)
	{
		switch (stayinMenu)
		{
			...
			case 'debug':
				...
				{
					boolText.text = (text.text == 'Botplay' ? (OnlinePlayState.instance.cpuControlled ? 'ON' : 'OFF') : (OnlinePlayState.instance.practiceMode ? 'ON' : 'OFF'));
				}
			case 'difficulty':
	...
	function doEvent()
	{
		if (stayinMenu == 'base')
		{
			...
			switch (daChoice)
			{
				...
				case 'Debug':
					...
					OnlinePlayState.chartingMode = true;
				case 'Options':
					OnlinePlayState.instance.paused = true; // For lua
					OnlinePlayState.instance.vocals.volume = 0;
				...
				case 'Restart':
					restartSong();
				case 'Exit':
					OnlinePlayState.deathCounter = 0;
					OnlinePlayState.seenCutscene = false;

					Mods.loadTopMod();
					if (OnlinePlayState.isStoryMode)
					{
						MusicBeatState.switchState(new StoryMenuState());
					}
					else
					{
						OnlinePlayState.replayMode = false;
					...
					OnlinePlayState.changedDifficulty = false;
					OnlinePlayState.chartingMode = false;
		...
		else if (stayinMenu == 'debug')
		{
			...
			{
				case 'Botplay':
					OnlinePlayState.instance.cpuControlled = !OnlinePlayState.instance.cpuControlled;
					OnlinePlayState.changedDifficulty = true;
					OnlinePlayState.instance.botplayTxt.visible = OnlinePlayState.instance.cpuControlled;
					OnlinePlayState.instance.botplayTxt.alpha = 1;
					OnlinePlayState.instance.txtSine = 0;
					boolText.text = (OnlinePlayState.instance.cpuControlled ? 'ON' : 'OFF');
				case 'Practice':
					OnlinePlayState.instance.practiceMode = !OnlinePlayState.instance.practiceMode;
					OnlinePlayState.changedDifficulty = true;
					boolText.text = (OnlinePlayState.instance.practiceMode ? 'ON' : 'OFF');
				case 'Skip Time':
					if (curTime < Conductor.songPosition)
					{
						OnlinePlayState.startOnTime = curTime;
						restartSong(true);
					}
					else
					{
						if (curTime != Conductor.songPosition)
						{
							OnlinePlayState.instance.clearNotesBefore(curTime);
							OnlinePlayState.instance.setSongTime(curTime);
						}
						close();
					}
				case 'Leave':
		...
		else if (stayinMenu == 'difficulty')
		{
			...
			closeMenu(function(trm:FlxTimer)
			{
				try
				{
					var name:String = OnlinePlayState.SONG.song;
					var poop = Highscore.formatSong(name, difficultyCurSelected);
					OnlinePlayState.SONG = Song.loadFromJson(poop, name);
					OnlinePlayState.storyDifficulty = difficultyCurSelected;
				...
				{
					missingText.text = 'ERROR WHILE LOADING CHART: ' + OnlinePlayState.SONG.song + '-' + difficultyChoices[difficultyCurSelected];
