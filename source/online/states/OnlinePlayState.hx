...
class OnlinePlayState extends MusicBeatState
{
	...
	function createPauseButton()
	{
		...
		pauseButton = new FlxSprite(2, 2).loadGraphic(Paths.image('menuExtend/PlayState/pauseButton'));
		pauseButton.setGraphicSize(100, 100);
		pauseButton.alpha = 0.5;
		pauseButton.scrollFactor.set();
		pauseButton.camera = pauseCam;
	...
	override public function update(elapsed:Float)
	{
		...
		if (canPause)
		{
			...
			{
				var realTouch = touch.getScreenPosition(pauseCam);
				if (realTouch.y >= pauseButton.y
					&& realTouch.y <= pauseButton.y + pauseButton.height
					&& realTouch.x >= pauseButton.x
					&& realTouch.x <= pauseButton.x + pauseButton.width
					&& touch.justPressed)
				{
					openPauseMenu();
					break;
